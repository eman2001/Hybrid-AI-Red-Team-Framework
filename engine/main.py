...
def run_owasp_web_scan(target_url):
    ...
    # Bounded, same-host discovery pass (links, forms, robots.txt,
    # sitemap.xml, common REST-path probe) so checkers aren't limited to
    # whatever's linked from the bare homepage. This is what SPAs like
    # Juice Shop need -- their rendered HTML has almost no <a href="?...">
    # links, so without this every checker was only ever testing "/".
    from engine.modules.web_security.web_discovery import WebDiscovery
    try:
        discovery = WebDiscovery(target_url).discover()
        discovered_endpoints = discovery["endpoints"]
        print(f"  [OWASP] Discovery: {discovery['pages_visited']} page(s) visited, "
              f"{len(discovered_endpoints)} endpoint(s), {len(discovery['forms'])} form(s)")
    except Exception as e:
        print(f"  [OWASP] Discovery failed ({e}) -- falling back to homepage-only testing")
        discovered_endpoints = []

    # Register all OWASP checkers
    engine.register_checker(InjectionChecker(target_url, discovered_endpoints=discovered_endpoints))
...
def run_pipeline(target: str, lhost: str):
    ...
    # If SMB (139/445) is open, try anonymous-share username discovery --
    # best-effort, read-only. Any names found are passed to Hydra later
    # (Phase 6) alongside its standard default-credential list.
    discovered_usernames = []
    has_smb = any(
        isinstance(host_data, dict)
        and any(p.get("port") in (139, 445) for p in host_data.get("ports", []))
        for host_data in scan_results.values()
    )
    if has_smb:
        from engine.modules.recon.smb_enum import enumerate_anonymous_usernames
        print("\n[Recon] SMB detected -- checking for anonymous share access...")
        discovered_usernames = enumerate_anonymous_usernames(live_hosts[0])
        if discovered_usernames:
            print(f"  [+] Discovered {len(discovered_usernames)} candidate username(s) "
                  f"from anonymous SMB share(s): {', '.join(discovered_usernames)}")
        else:
            print("  [-] No anonymous SMB access or no usable text files found")
    ...
    # ── Dedup: merge OWASP findings that are exact duplicates ───────────
    # (same host, port, CWE/CVE id, and title) into one canonical finding
    # with a merged evidence/description list, instead of feeding N
    # identical entries into Threat Intel / Risk Scoring / MITRE downstream.
    # Additive only — does not touch findings from vuln_mapper (Nmap/EDB).
    def _dedup_owasp_findings(findings: list[dict]) -> list[dict]:
        owasp_ones   = [f for f in findings if f.get('source') == 'OWASP Web Security']
        non_owasp    = [f for f in findings if f.get('source') != 'OWASP Web Security']

        merged: dict[tuple, dict] = {}
        for f in owasp_ones:
            key = (f['host'], f['port'], f.get('cve', ''), f.get('vulnerability', ''))
            if key not in merged:
                merged[key] = {**f, '_duplicate_count': 1, '_evidence': [f.get('description', '')]}
            else:
                merged[key]['_duplicate_count'] += 1
                if f.get('description') and f['description'] not in merged[key]['_evidence']:
                    merged[key]['_evidence'].append(f['description'])

        deduped = list(merged.values())
        removed = len(owasp_ones) - len(deduped)
        if removed:
            print(f"  [Dedup] Merged {removed} duplicate OWASP finding(s) "
                  f"({len(owasp_ones)} -> {len(deduped)})")
        return non_owasp + deduped

    vuln_findings = _dedup_owasp_findings(vuln_findings)
    ...
    for result in exploit_results:
        if result.get("session_obtained"):
            ...
            log.info(
                ...
                result.get("winning_method"), result.get("host"), result.get("port"),
            )

    sessions_available = sum(1 for r in exploit_results if r.get("session_obtained"))
    post_completed = 1 if post_data.get("session_obtained") else 0
    print(f"  [Summary] Sessions available : {sessions_available}")
    print(f"  [Summary] Post-exploitation completed : {post_completed}")
    if sessions_available == 0:
        print("  [Summary] Post-exploitation skipped — no interactive session was "
              "obtained in Phase 6 (this is expected/correct when Metasploit RPC "
              "is unavailable or no exploit produced a session; it is not a "
              "pipeline failure).")
