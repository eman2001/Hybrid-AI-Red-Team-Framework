...
from engine.modules.api.current_report import set_latest_session
...
def run_pipeline(target: str, lhost: str):
    ...
    # ── Evaluation Metric: Automation Level ──────────────────────────
    total_pipeline_phases = 12
    automated_phases_completed = 0

    # Reserved for later direct measurement of operator intervention.
    # It is kept in the output schema but is not used in the automation formula.
    manual_intervention_count = 0

    add_activity(
        title="New Scan Started",
        description=f"Scanning target {target}",
    ...
    log.info(
        "Session %s started — target=%s lhost=%s",
        session_id,
        target,
        lhost,
    )
    ...
    if not live_hosts:
        print("[-] No live hosts. Halting.")
        return

    automated_phases_completed += 1
    ...
    if owasp_results.get("owasp_findings"):
        ...
        print(
            f"  [+] Added "
            f"{len(owasp_results['owasp_findings'])} "
            f"OWASP findings"
        )

    if not vuln_findings:
        print("[-] No vulnerabilities mapped. Halting.")
        return
    ...
    high_risk_findings = risk_engine.filter_by_risk(
        vuln_findings,
        threshold=30,
    )

    if not high_risk_findings:
        print("[-] No high-risk targets. Halting.")
        return
    ...
    # ── Evaluation Metric: Exploitation Success Rate ──────────────────
    total_exploit_attempts = len(exploit_results)
    ...
    if total_exploit_attempts > 0:
        exploitation_success_rate = round(
            (successful_exploits / total_exploit_attempts) * 100,
            2,
        )
    else:
        exploitation_success_rate = None

    if exploitation_success_rate is not None:
        print(
            f"[METRIC] Exploitation Success Rate: "
            f"{successful_exploits}/{total_exploit_attempts} "
            f"({exploitation_success_rate:.2f}%)"
        )
    else:
        print(
            "[METRIC] Exploitation Success Rate: "
            "N/A (no exploitation attempts)"
        )

    log.info(
        "Exploitation complete — %d attempts, %d successful",
        total_exploit_attempts,
        successful_exploits,
    )
    ...
    add_activity(
        ...
        description=(
            f"{total_exploit_attempts} exploitation attempts executed — "
            f"{successful_exploits} successful"
        ),
        activity_type="warning",
        icon="target",
    )
    ...
    for result in exploit_results:
        if result.get("session_obtained"):
            ...
            # attack_chain=None — integrator runs after Phase 8
            ...
            if post_data.get("session_obtained"):
                post_commands = [
                    "sysinfo",
                    "getuid",
                    "hashdump",
    ...
    mapped_results = mitre_engine.map_all(
        exploit_results,
    ...
    if post_data:
        attack_chain = AttackChainIntegrator().integrate(
            post_data,
            attack_chain,
        )
        log.info(
            "Attack chain extended: %d phases",
            len(attack_chain),
        )

    mitre_engine.save_heatmap(
        mapped_results,
        f"reports/attack_layer_{ts}.json",
    )

    mitre_engine.save_chain(
        attack_chain,
        f"reports/attack_chain_{ts}.json",
    )
    ...
    print(
        f"  [Coverage] Tactics={len(coverage['covered_tactics'])} "
        f"Techniques={coverage['technique_count']} "
        f"Coverage={coverage['tactic_coverage_pct']}%"
    )

    # SigmaMapper — detection recommendations
    technique_ids = [
        m.get("mitre", {}).get("technique_id", "")
    ...
    if technique_ids:
        ...
        log.info(
            "[Detection] Sigma rules mapped: %d",
            len(sigma_results),
        )

        print(
            f"  [Sigma] "
            f"{len(sigma_results)} detection rules generated"
        )
    else:
        sigma_results = []
    ...
    print(
        "\n[Phase 9] AI Enrichment "
        "(MitrePredictor · XAI · Adversary · Recommendations)"
    )

    ai_pipeline = AIPipeline()
    ai_results = ai_pipeline.enrich_findings(
        mapped_results,
        attack_chain,
    )
    ...
    if ai_results.get("adversary_match"):
        ...
        print(
            f"  [AI] Closest adversary: {top.get('apt')} "
    ...
    if ai_results.get("recommendations"):
        print(
            f"  [AI] "
            f"{len(ai_results['recommendations'])} "
    ...
    print(
        f"  [Graph] Nodes={graph_analysis.get('total_nodes', 0)} "
    ...
    se_module = SocialEngineeringModule(
        domain,
        lhost,
    )

    se_results = se_module.run_campaign({
    ...
    print(
        "\n[Phase 12] "
        "Generating Reports & Saving to Database"
    )
    ...
    risk_summary = {
        ...
        "post_credentials": len(
            post_data.get("hashes", [])
        ),
        "lateral_targets": len(
            post_data.get("lateral_opps", [])
        ),
        "owasp_findings": len(
            owasp_results.get(
                "owasp_findings",
        ...
        "technologies": owasp_results.get(
            "technologies",
            [],
        ),
    }
    ...
    # ── Evaluation Metric 1: Execution Time ──────────────────────────
    pipeline_end = time.perf_counter()
    execution_time = pipeline_end - pipeline_start

    print(
        f"[METRIC] Total Execution Time: "
        f"{execution_time:.2f} seconds"
    )

    # ── Evaluation Metric: Automation Level ──────────────────────────
    completed_pipeline_phases = automated_phases_completed

    if completed_pipeline_phases > 0:
        automation_level = 100.0
    else:
        automation_level = None

    phase_completion_rate = round(
        (
            completed_pipeline_phases
            / total_pipeline_phases
        ) * 100,
        2,
    )
    ...
    if automation_level is not None:
        print(
            f"[METRIC] Automation Level: "
            f"{automation_level:.2f}% "
            f"({automated_phases_completed} automated phases)"
        )

    print(
        f"[METRIC] Pipeline Completion: "
        f"{completed_pipeline_phases}/"
        f"{total_pipeline_phases} "
        f"({phase_completion_rate:.2f}%)"
    )
    ...
    print(
        f"[+] Findings      : "
        f"{len(vuln_findings)} vulnerabilities"
    )
    print(
        f"[+] OWASP Findings: "
        f"{len(owasp_results.get('owasp_findings', []))}"
    )
    print(
        f"[+] Technologies  : "
        f"{', '.join(owasp_results.get('technologies', []))}"
    )
    print(
        f"[+] MITRE Techs   : "
        f"{len(merged_techs)} unique techniques"
    )
    print(f"[+] Chain Phases  : {len(attack_chain)}")
    print(
        f"[+] MITRE Coverage: "
        f"{coverage['tactic_coverage_pct']}%"
    )
    print(
        f"[+] Graph Nodes   : "
        f"{graph_analysis.get('total_nodes', 0)}"
    )
    print(
        f"[+] AI Recs       : "
        f"{len(ai_results.get('recommendations', []))}"
    )
    ...
    print(
        f"[+] Exploit Attempts: "
        f"{total_exploit_attempts}"
    )
    print(
        f"[+] Exploit Success : "
        f"{successful_exploits}"
    )
    ...
    if exploitation_success_rate is not None:
        ...
        print("[+] Exploit Rate    : N/A")

    print(f"{'=' * 62}")

    # IMPORTANT: finish() must run before return
    finish()
    ...
    return {
        ...
        "evaluation_metrics": {
            ...
            "automation": {
                "total_pipeline_phases": total_pipeline_phases,
                "automated_phases_completed": automated_phases_completed,
                "manual_interventions": manual_intervention_count,
                "automation_level_pct": automation_level,
                "pipeline_completion_pct": phase_completion_rate,
            },

            "exploitation": {
                "total_attempts": total_exploit_attempts,
                "successful_attempts": successful_exploits,
                "success_rate_pct": exploitation_success_rate,
            },
        },
...
def main():
    ...
    target = input(
        ...
        "(e.g. 192.168.1.100 or http://site.com): "
    ).strip()
